...
export default function Faucet({
  ...
  const isDisabledButton = useMemo(() => {
    const { transactionType, tokenAmount, selectedToken, address, isAddressError } = formState;
    if(transactionType === TransactionType.address) return !address || statusContext.isLoading || !tokenAmount || !selectedToken || isAddressError;
    if(transactionType === TransactionType.wallet) return !user.userAddress || statusContext.isLoading || !tokenAmount || !selectedToken
      return true;
  },[formState, statusContext, user])

  const requestAddress = useMemo(() => formState.transactionType === TransactionType.address ? formState.address : user.userAddress,[formState, user])
  ...
  return (
    <div className="faucet-main-wrapper">
      ...
      <div className="faucet-container">
        ...
        {formState.transactionType === TransactionType.address && <AddressField
              status={statusContext}
              formState={formState}
              setFormState={setFormState}
            />}

        <TokenSelect formState={formState}
                     setFormState={setFormState} />
